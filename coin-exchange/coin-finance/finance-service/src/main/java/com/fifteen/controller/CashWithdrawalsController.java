...
public class CashWithdrawalsController {
    ...
    @GetMapping("/updateWithDrawalsStatus")
    ...
    @ApiOperation(value = "查询用户的提现记录")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "current", value = "当前页"),
            @ApiImplicitParam(name = "size", value = "每页显示的条数"),
            @ApiImplicitParam(name = "status", value = "提现审核状态")
    })
    public R<Page<CashWithdrawals>> findUserCashRecharge(@ApiIgnore Page<CashWithdrawals> page, Integer status) {
        Long userId = Long.valueOf(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        Page<CashWithdrawals> pageData = cashWithdrawalsService.findUserCashRecharge(page, userId, status);
        return R.ok(pageData);
    }
