...
public class AccountServiceImpl extends ServiceImpl<AccountMapper, Account>
    ...
    @Override
    public boolean transferAccountAmount(Long adminId, Long userId, Long coinId, Long orderNum, BigDecimal num, BigDecimal fee
            , String remark, String businessType, Integer direction) {
        Account coinAccount = getCoinAccount(coinId, userId);
        if (coinAccount == null) {
            throw new IllegalArgumentException("用户没有此币种的资产");
        }
        ...
        if (save) {
            ...
            boolean updateId = updateById(coinAccount);
            return updateId;
        }
        return save;
    }
    ...
    @Override
    public Boolean decreaseAccountAmount(Long adminId, Long userId, Long coinId, Long orderNum, BigDecimal num, BigDecimal fee, String remark, String businessType, Integer direction) {
        Account coinAccount = getCoinAccount(userId, coinId);
        if (coinAccount == null) {
            throw new IllegalArgumentException("用户没有此币种类的资产");
        }
        AccountDetail accountDetail = new AccountDetail();
        accountDetail.setUserId(userId);
        accountDetail.setCoinId(coinId);
        ...
        if (save) {
            ...
            BigDecimal result = balanceAmount.add(num.multiply(new BigDecimal(-1)));
